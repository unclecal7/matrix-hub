...
protected:
	MediaPoolTransfer(HWND m_hWnd, IBMDSwitcher* switcher, IBMDSwitcherMediaPool* mediaPool);
	virtual ~MediaPoolTransfer();
	
	IBMDSwitcherFrame*		loadFrame(CString filePath); // loads a frame from file
	CString					fileNameFromFilePath(const CString& filePath); // gets a file name from a file path

	HWND					mHWnd; // window handle for posting messages
...
// The main dialog passes windows messages received from the Stills and Lock callbacks to StillTransfer.
class StillTransfer : private MediaPoolTransfer
{
public:
	StillTransfer(HWND m_hWnd, IBMDSwitcher* switcher, IBMDSwitcherMediaPool* mediaPool, IBMDSwitcherStills* stills);
	~StillTransfer();

	// this example demonstrates uploading only, however downloading is very similar
	bool upload(int stillIndex, CString filePath);

	// the dialogs message queue reports back to us via the following methods
	void onLockObtained();
...
private:
	...
	CString					mTransferName;			// target still name
	IBMDSwitcherFrame*		mTransferFrame;			// frame to transfer
};

// The ClipTransfer class assists in uploading clips to the switcher.
// The main dialog passes windows messages received from the Clip and Lock callbacks to ClipTransfer.
class ClipTransfer : private MediaPoolTransfer
{
public:
	ClipTransfer(HWND m_hWnd, IBMDSwitcher* switcher, IBMDSwitcherMediaPool* mediaPool, IBMDSwitcherClip* clip);
	~ClipTransfer();

	// this example demonstrates uploading a series of images only, however downloading is very similar
	bool upload(const std::list<CString>& filePaths);
...
private:
	...
	// transfer properties
	bool					mTransferActive;		// true if a transfer is being processed
	std::list<CString>		mTransferFilePaths;		// list of unfinished files to upload
	CString					mTransferName;			// target clip name
	IBMDSwitcherFrame*		mTransferFrame;			// frame to transfer
