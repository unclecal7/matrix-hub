...
void						print_recording_drives(com_ptr<IBMDSwitcherRecordAV>& switcherRecord);
void						print_hyperdecks(com_ptr<IBMDSwitcher>& switcher);
...
int main (int argc, char** argv)
{
	...
	com_ptr<IBMDSwitcherStreamRTMP>						switcherStream;
	com_ptr<IBMDSwitcherRecordAV>						switcherRecord;
	...
	switcherStream = com_ptr<IBMDSwitcherStreamRTMP>(IID_IBMDSwitcherStreamRTMP, switcher);
	printf(" %-40s %s\n", "Supports Streaming:", switcherStream ? "Yes" : "No");

	switcherRecord = com_ptr<IBMDSwitcherRecordAV>(IID_IBMDSwitcherRecordAV, switcher);
...
void print_recording_drives(com_ptr<IBMDSwitcherRecordAV>& switcherRecord)
{
	com_ptr<IBMDSwitcherRecordDiskIterator> diskIterator;
	com_ptr<IBMDSwitcherRecordDisk> disk;
	uint32_t totalTime = 0;

	printf("\nAvailable Recording Drives:\n");
	printf(" %-40s%s\n", "Name", "Recording Time Available");

	if (switcherRecord->CreateIterator(IID_IBMDSwitcherRecordDiskIterator, (void**)diskIterator.releaseAndGetAddressOf()) == S_OK)
	{
		while (diskIterator->Next(disk.releaseAndGetAddressOf()) == S_OK)
		{
			CFStringRef	diskName;
			uint32_t timeAvailable;
			if ((disk->GetVolumeName(&diskName) == S_OK) &&
				(disk->GetRecordingTimeAvailable(&timeAvailable) == S_OK))
			{
				printf(" %-40s%02u:%02u:%02u\n", CFStringToStdString(diskName).c_str(),
...
void print_hyperdecks(com_ptr<IBMDSwitcher>& switcher)
{
	com_ptr<IBMDSwitcherHyperDeckIterator> hyperdeckIterator;
	com_ptr<IBMDSwitcherHyperDeck> hyperDeck;

	printf("\nConnected HyperDecks:\n");
	printf(" %-17s%-15s%-35s%s\n", "IP Address", "Connection", "Model", "Clip Count");

	if (switcher->CreateIterator(IID_IBMDSwitcherHyperDeckIterator, (void**)hyperdeckIterator.releaseAndGetAddressOf()) == S_OK)
	{
		while (hyperdeckIterator->Next(hyperDeck.releaseAndGetAddressOf()) == S_OK)
		{
			...
			if (hyperDeck->GetConnectionStatus(&status) == S_OK && status != bmdSwitcherHyperDeckConnectionStatusNotConnected)
			{
				...
				printf(" %-17s%-15s%-35s%u\n",
					   ...
					   statusString.c_str(),
					   CFStringToStdString(modelName).c_str(),
					   clipCount);
